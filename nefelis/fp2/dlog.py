...
class Descent:
    ...
    def _factorg(self, x, y):
        ...
        facs = integers.factor(self.norm(x, y))
        for l, e in facs:
    ...
    def smooth_candidates(self, x0):
        ...
        for i in range(1_000_000):
            ...
            if max(a.bit_length(), b.bit_length()) < n.bit_length() // 2:
                ...
                n_fractions = 0
                for q, qr in zip(ls[10:40], lr1[10:40]):
                    reports = sieve.sieve(q, qr)
                    logger.info(f"Vector sieve {q},{qr}: {len(reports)} reports")
    ...
    def smalllog(self, q: int, qr: int) -> int:
        ...
        for qq, qqr in q2s:
            ...
            assert qrcomp % q == qr and qrcomp % qq == qqr, (
                q,
                qr,
                qq,
                qqr,
            ...
            dt = time.monotonic() - t
            logger.info(f"Found {len(reports)} sieve results (q={q}*{qq}) in {dt:.3}s")
            time.sleep(1)
...
if __name__ == "__main__":
    ...
    logging.getLogger("vulkan").setLevel(level=logging.DEBUG)
    main()
